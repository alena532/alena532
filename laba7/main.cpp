...
void List::Push_back(FILE *file)
{
    ...
    fscanf(file,"%s%s%s%s%s%s",temp->flightNumber,temp->planeType,temp->country,temp->interCountry,temp->time,temp->days);
...
void List::Delete_list(FILE *file2)
{
    while(head!=NULL){
        Node *temp=head;
            fprintf(file2,"%s %s %s %s %s %s",temp->flightNumber,temp->planeType,temp->country,temp->interCountry,temp->time,temp->days);
        if(head->next!=NULL){
            fprintf(file2,"\n");
        }
            head=head->next;
...
{
    FILE *file2;
    if ((file2= fopen("/Users/alenavorobej/Documents/c/laba7/output.txt","w")) == NULL){
        cout<<"Mistake";
        exit(0);
        }
    a.Delete_list(file2);
